...
export default function Home({ user, onLogout, onSelectProject }) {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            alert('Error creating project. Make sure backend is running.');
        }
    };
                        ...
                        {projects.map(p => (
                            <div key={p.id} style={styles.card} onClick={() => onSelectProject(p)}>
                                <div style={styles.cardHeader}>
                                    ...
                                    <span style={{
                                        ...styles.badge,
                                        background: p.status === 'active' ? '#27ae60' : '#e67e22'
                                    }}>
                                        {p.status}
                                    </span>
                                </div>
                                ...
                                <div style={styles.budget}>
                                    ...
                                    <span style={styles.budgetValue}>
                                        KES {Number(p.budget).toLocaleString()}
                                    </span>
                                </div>
                            </div>
                        ))}
...
const styles = {
    ...
    card: { background: '#16213e', borderRadius: 12, padding: 20, cursor: 'pointer', border: '1px solid #333' },
